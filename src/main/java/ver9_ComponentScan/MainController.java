    ...
    help();
		...
		while(true) {
			...
			if(userInput.startsWith("new")){
				...
				MemberRegisterSerivce memRegSrv = ctx.getBean(MemberRegisterSerivce.class);
				...
				MemberPwdChangeService memPwdChgSrv = ctx.getBean(MemberPwdChangeService.class);
...
//        memPwdChgSrv.chgPwd(memDto);
			    ...
			    //@Service 애노테이션의 속성값으로 지정해준 Bean 이름으로 지정한다.
					MemberListService memListSrv = ctx.getBean("listService",MemberListService.class);
        ...
        //@Service 애노테이션의 속성값으로 지정해준 Bean 이름으로 지정한다.
			  MemberInfoService memInfoSrv = ctx.getBean("infoService",MemberInfoService.class);
        String email = userInputs[1];
